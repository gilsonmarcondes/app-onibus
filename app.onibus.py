...
with st.sidebar:
    ...
    st.caption("v6.5 · London Marathon Prep")
    st.divider()
...
aba_rota, aba_monitor, aba_ponto, aba_london = st.tabs([
    "🗺️ Planejador", "🚌 Monitor", "📍 Radar", "🇬🇧 Londres"
])
...
with aba_rota:
    ...
    with col_a:
        ...
        if t_o == "📍 Meu GPS":
            ...
            st.text_input("Local de saída (Digite e aperte ENTER):", placeholder="Ex: Metrô Ana Rosa", key="in_o", on_change=proc_origem)
            
            if st.session_state.get('busca_o_res'):
    ...
    if st.session_state.get('rota_ativa'):
        ...
        with c1:
            ...
            if st.button("🗑️ Nova Busca"):
                ...
                for k in ['origem_sel', 'destino_sel', 'busca_o_res', 'busca_d_res']: st.session_state[k] = None
                st.rerun()
        with c2:
    ...
    if lin_id and TOKEN_SPTRANS:
        ...
        if res_l:
            ...
            if vs:
                ...
                st_folium(m_f, width=1000, height=450, key="mapa_frota_v65")
